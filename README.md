...
# Upstream Configuration (if not using config files)
UPSTREAM_USERNAME=admin
UPSTREAM_TOKEN=your_token
UPSTREAM_PID=your_pid
UPSTREAM_SOCKET_URL=ws://ref.petrolimexaviation.vn:36300/ws
```
...
Update `src/config/upstream.json` with your upstream service credentials:
...
1. **Upstream Connection Failed**
    ...
    - Check credentials in `src/config/upstream.json`
